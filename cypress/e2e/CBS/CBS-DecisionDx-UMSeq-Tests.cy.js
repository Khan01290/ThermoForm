...
describe('DecisionDx-UMSeq Online Order Submission Suite', function() {

    it.skip("Verify that DecisionDx-UMSeq: Online Order Created and Submitted Successfully", function()  {
    ...
    it("Verify DecisionDx-UMSeq Test Created Send for approval and Submitted Status Flow", function()  {
        ...
        cy.contains('p', 'DecisionDx-UMSeq').scrollIntoView().should('be.visible').and('exist').click().wait(500)                                                          // Tapping to MyPath Melanoma Test        
        ...
        */
        ...
        cy.wrap($options).each(($option, index) => {
            ...
            const randomIndexcl = Math.floor(Math.random() * $options.length)
            const chosen = $options[randomIndexcl]
            cy.wrap($options[randomIndexcl])
            cy.wrap(chosen).click({ force: true})
            cy.log('Randomly selected option: ' + $options[randomIndexcl].value)
        })
        ...
        cy.window().then((win) => {
            ...
            if (eRequestId) {
            // ✅ SUCCESS
            cy.log(`✅ eRequestId generated: ${eRequestId} for order id ${orderId}`)
    ...
    it.skip("Verify DecisionDx-UMSeq Test Created with Save and Exit Flow", function()  {
        ...
        cy.get('path[data-name="Path 5122"]').each(($el) => {                                                                                                             // Asserting each sidebar circle icon color in not filled.
        cy.wrap($el).should('have.attr', 'fill', 'none')})
        cy.get(':nth-child(4) > .rounded-md > .py-4 > .flex-wrap > :nth-child(2) > .flex > .font-medium').scrollIntoView().should('have.text','DecisionDx-UMSeq')         // Tapping to DecisionDx-UMSeq Test
        .should('be.visible').and('exist').click().wait(500)
        cy.get('#securityInfo\\.ssn').type('232-32-3232')                                                                                                                 // Input SSN
        cy.get(':nth-child(2) > .primaryBtn').click().wait(500)                                                                                                           // Tap to Proceed to order btn
        cy.get('path[data-name="Path 5122"]').should('have.attr', 'fill', '#18B5B7')                                                                                    // Asserting side bar Select text icon color is filled  
